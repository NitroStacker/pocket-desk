...
- `input` contains one validated, allowlisted mouse, keyboard, window-focus, text, or application-specific action. Aurora FX actions are restricted to named lighting zones, bounded values, known effects, valid RGB colors, and validated custom LED syntax.
...
The mobile client selects a renderer from a shared adapter registry. File Explorer, Bezi, Chrome, ChatGPT/Codex, Windows Settings, Notepad/document apps, Camera, and Aurora FX each have layouts built from their actual accessibility controls and current window geometry. Aurora FX adds a code-native case-zone diagram, HSV picker, effect controls, and sliders; validated commands cross a local named pipe to the running Aurora FX process, which owns all AW-ELC device access, while semantic scans provide live readback. Camera and visual-heavy surfaces also use authenticated per-window visual capture; Camera refreshes its preview independently while its shutter and adjustment controls remain semantic. The Windows host keeps a dedicated, serialized DirectShow `IAMCameraControl` bridge alive for low-latency standard-UVC pan, tilt, and zoom. PIXY firmware 2.x does not advertise or acknowledge an independent status-light control, so the host exposes that capability as unavailable instead of treating a completed USB write as a physical light change. Camera commands are allow-listed and range-clamped, and the three user-saved PTZ presets persist under the current Windows user's local application data. Other applications use the universal semantic reflow and visual-region fallback. Activating any reconstructed control or visual region operates the corresponding element on the real desktop and requests a fresh reflow. Protected credential, lock, consent, and shell helper surfaces remain excluded.
